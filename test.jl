...
O_on_site = on_site_terms(Z, fuser); # Sum the tuple of 3 operators into a single operator
O1_two_site, O2_two_site = two_site_terms(X, Y, Z, my_id, fuser);
...
function my_hummble_fixedpoint(
    ...
    (peps_final, env_final), cost_final, ∂cost, numfg, convergence_history = optimize(
        ...
        peps = InfinitePEPS([convert_peps(peps[r,c], bond_matrix, fuser) for r in 1:size(peps, 1), c in 1:size(peps, 2)]);
        start_time = time_ns()
